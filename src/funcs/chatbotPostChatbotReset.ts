 ...
 * Reset Token
 */
export async function chatbotPostChatbotReset(
  client: IsoscelesCore,
  request: operations.PostChatbotResetRequest,
  options?: RequestOptions,
): Promise<
  Result<
    components.ChatbotResponse,
    | SDKError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  ...
  const path = pathToFunc("/chatbot/reset")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, components.ChatbotResponse$inboundSchema),
    M.fail(["4XX", "5XX"]),
  )(response);
  if (!result.ok) {
    return result;
