 ...
 * Download
 */
export async function documentDownloadDocument(
...
> {
  ...
  const path = pathToFunc("/document/{document_id}/download")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, operations.DownloadDocumentResponseBody$inboundSchema),
    M.fail(["4XX", "5XX"]),
  )(response);
  if (!result.ok) {
    return result;
